/**
  ...
  * @version 0.2.0
  * Prompt the user for the song they want to play and plays it, displaying artwork too.
  */
...
public class PlayMusic
{
   ...
   /**
     * Runs the PlayMusic program, prompting for various inputs, playing music, and displaying artwork.
   ...
   public static void main(String[] args)
   {
      ...
      try /* Possible Exceptions from IO */
      {
         ...
         while(!song.getFilename().equals("exit")) /* If the user wants to play a song. */
         {
            
            /* Play music of choice */
            
            if(kawaii.isPlaying()) /* If playing: stop */
            {
               kawaii.stopMusic();
            }
            ...
            if(painted)
            {
               frame.repaint(song.getTitle(), new File(picdir + song.getArtFilename() + picExt));
               frame.setVisible(true);
            }
            else
            {
               frame = new ArtFrame(song.getTitle(), 485, 510, new File(picdir + song.getArtFilename() + picExt));
   ...
   /**
     ...
     * @throws IOException Throws an exception if something goes wrong while reading the songlist file.
	  */
   private static ArrayList<SongDetails> buildSonglist(String filename) throws IOException
   {
      try
      {         
         ...
         while(songin.hasNextLine())
         {
            ...
            if(!songin.hasNextLine())
               throw new IOException(String.format("No music filename for file, %s.", title));
   ...
   /**
     ...
     * @throws IOException Throws an exception if something goes wrong while reading the songlist file.
	  */
   private static ArrayList<SongDetails> buildSonglist2(String filename) throws IOException
   {
      try
      {         
         ArrayList<SongDetails> songinfos = new ArrayList<SongDetails>();
   ...
   private static Scanner getPlaylist(int songCount)
   {
      ...
      while(true) /* Until valid choice */
      {
         ...
         System.out.printf("Which playlist source?\n"
                         ...
                         +"2) Create one: Create a playlist of songs\n\t(Note: you can save this playlist as a file too)\n"
   ...
   private static void savePlaylist(String playlist)
   {
      ...
      {
         System.out.printf("Unable to save playlist to file, %s. Try again later.", filename);
      }
   }
   
   /**
     * Makes a playlist from the selected file containing a playlist
     * @return Returns a Scanner that scans over all the songs specified in the file.
     */
   private static Scanner getFilePlaylist()
   {
      ...
      {
         System.out.printf("Unable to open file, %s. Reverting back to custom control.\n", filename);
         return input; /* Returns system.in scanner if file failed to open. */
      } 
   }
   
   /**
     * Makes a playlist containing a random number, r, of songs in random order. Where r is greater than 5 and less than 100.
     * @param songCount The total number of songs available including "0", exit.
     * @return Returns a Scanner that scans over at least 5 songs in random order.
     */
   private static Scanner makeShufflePlaylist(int songCount)
   {
      StringBuilder all = new StringBuilder();
      int play_count = (int)(Math.random() * 95 + 5); /* Gets number of songs to play */
      
      System.out.printf("Playing %d shuffled songs.\n\n", play_count); /* Prints the number of songs to be played. */
      
      /* Builds string playlist of random ints, i. Where 0 < i < songCount */
      for(int i = 1; i < play_count; i++)
         all.append(String.format("%d ", (int)(Math.random()*(songCount-1)+1)));
      all.append("0");
   ...
   /**
     ...
     * @param songinfos ArrayList of Song Details to be used for displaying the song options by title 
     *                  and for getting the filename of the choice to return. 
     * @return String of file name with out .extension.
     */
   private static SongDetails getSongChoice(Scanner playlist, ArrayList<SongDetails> songinfos)
   {
      int choice;
      while(true)
      {         
   ...
   /**
     * Creates a songlist file with the, with the given name, containing the contents of the given ArrayList of SongDetails 
     * @param filename A String for the name of the file to save the songlist to.
     * @param songlist An ArrayList of SongDetails that will be printed out to the created file.
     * @throws IOException Throws an IOException if the file is unable to open for writing.
     */
   private static void saveSonglist(String filename, ArrayList<SongDetails> songlist) throws IOException
   {
      try
      {
   ...
   /**
     * Sorts the given ArrayList of SongDetails in the order specified by the SongDetails' compareTo().
     * New list will be return, and the original list will remain unchanged. 
     * Uses a QuickSort algorithm to sort the list.
     * @param songlist The ArrayList of SongDetails to be sorted. Will remain unchanged.
     * @return Returns a copy of the given ArrayList that is sorted and independent of the original.
     */
   @SuppressWarnings("unchecked")
   private static ArrayList<SongDetails> sortSonglist(ArrayList<SongDetails> songlist)
   {
